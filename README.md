...
alt="Figure 5: All the functions for relative expression analysis (including TTEST_DDCt, WILCOX_DDCt, ANOVA_DDCt, and ANOVA_DCt) return the relative expression table which include fold change and corresponding statistics. The output of ANOVA_DDCt, and ANOVA_DCt also include lm model, residuals, raw data and ANOVA table for each gene." />
<figcaption aria-hidden="true">Figure 5: All the functions for
